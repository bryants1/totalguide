...
try:
    ...
    print("✅ OSMnx available - will try name→boundary first")
except ImportError:
    OSMNX_AVAILABLE = False
    print("⚠️ OSMnx not available - install with: pip install osmnx")

try:
    from shapely import union_all  # Shapely 2.x
except Exception:
    # Fallback for Shapely 1.x environments
...
class GeospatialVisualizer:
    ...
    def __init__(self, center_lat: float, center_lon: float, extent_km: float = 2.0,
        ...
        self.course_name_raw = course_name
        ...
        self.course_boundary_file = os.path.join(self.course_folder, "course_boundary.geojson")
    ...
    def _best_polygon_from_gdf(self, gdf: gpd.GeoDataFrame) -> Optional[gpd.GeoDataFrame]:
        """Pick the largest polygon/multipolygon from a GDF."""
        if gdf is None or gdf.empty:
            return None
        gdf_poly = gdf[gdf.geometry.type.isin(["Polygon", "MultiPolygon"])].copy()
        if gdf_poly.empty:
            return None
        gdf_poly["__area"] = gdf_poly.to_crs(epsg=3857).geometry.area
        largest = gdf_poly.sort_values("__area", ascending=False).head(1).drop(columns="__area")
        return largest.to_crs(epsg=4326)

    def _buffer_point_gdf(self, gdf: gpd.GeoDataFrame, buffer_m: float = 600.0) -> Optional[gpd.GeoDataFrame]:
        """If we only geocode to a point/line, buffer it to a polygon in meters."""
        if gdf is None or gdf.empty:
            return None
        gdf_m = gdf.to_crs(epsg=3857)
        buffered = gdf_m.buffer(buffer_m)
        poly = gpd.GeoDataFrame(geometry=[buffered.unary_union], crs=gdf_m.crs).to_crs(epsg=4326)
        return poly

    def _save_boundary_cache(self, boundary_gdf: gpd.GeoDataFrame) -> None:
        try:
            boundary_gdf.to_file(self.course_boundary_file, driver="GeoJSON")
            print(f"💾 Cached course boundary → {self.course_boundary_file}")
        except Exception as e:
            print(f"⚠️ Could not save boundary cache: {e}")

    def _load_boundary_cache(self) -> Optional[gpd.GeoDataFrame]:
        try:
            if os.path.exists(self.course_boundary_file):
                gdf = gpd.read_file(self.course_boundary_file)
                if not gdf.empty:
                    print(f"✅ Loaded cached boundary from {self.course_boundary_file}")
                    return gdf
        except Exception as e:
            print(f"⚠️ Could not read boundary cache: {e}")
        return None

    def _ox_features_from_polygon(self, poly, tags):
        """
        OSMnx 1.x uses geometries_from_polygon
        OSMnx 2.x uses features_from_polygon
        This shim calls whichever exists.
        """
        if not OSMNX_AVAILABLE:
            return None

        # Ensure we pass a shapely (multi)polygon
        # (union_all handles GeoSeries/iterables of geometries)
        try:
            from shapely.geometry import Polygon, MultiPolygon
            if hasattr(poly, "geom_type"):
                geom = poly  # already a shapely geometry
            else:
                # if it's a GeoSeries or iterable, merge
                geom = union_all(list(poly))
        except Exception:
            geom = poly

        # Prefer v2 API if present, else v1
        if hasattr(ox, "features_from_polygon"):
            return ox.features_from_polygon(geom, tags)
        if hasattr(ox, "geometries_from_polygon"):
            return ox.geometries_from_polygon(geom, tags)

        raise AttributeError("No suitable OSMnx polygon fetch function found in this environment.")


    def _find_course_boundary_by_name(self) -> Optional[gpd.GeoDataFrame]:
        if not OSMNX_AVAILABLE:
            return None

        # Try human-readable variants
        base = self.course_name_raw.strip()
        name_variants = [
            base,
            f"{base}, Massachusetts",
            f"{base} golf",
            f"{base} golf course",
        ]
        ...
        for variant in name_variants:
            try:
                print(f"🔎 OSMnx: geocoding '{variant}'")
                g = ox.geocode_to_gdf(variant)
                if g is None or g.empty:
                    continue

                # Prefer polygons, largest by area
                g_poly = g[g.geometry.type.isin(["Polygon", "MultiPolygon"])].copy()
                if g_poly.empty:
                    # buffer points/lines to coarse polygon
                    g_m = g.to_crs(epsg=3857)
                    buffered = g_m.buffer(600.0)  # 600 m coarse search
                    boundary = gpd.GeoDataFrame(geometry=[union_all(list(buffered))], crs=g_m.crs).to_crs(epsg=4326)
                else:
                    g_poly["__area"] = g_poly.to_crs(epsg=3857).geometry.area
                    boundary = g_poly.sort_values("__area", ascending=False).head(1).drop(columns="__area").to_crs(epsg=4326)

                # Try to refine to explicit leisure=golf_course polygon within boundary
                poly = union_all(list(boundary.geometry))
                try:
                    print("   📦 Querying for explicit leisure=golf_course within boundary")
                    gcourses = self._ox_features_from_polygon(poly, {"leisure": "golf_course"})
                    if gcourses is not None and not gcourses.empty:
                        gc_poly = gcourses[gcourses.geometry.type.isin(["Polygon","MultiPolygon"])].copy()
                        if not gc_poly.empty:
                            gc_poly["__area"] = gc_poly.to_crs(epsg=3857).geometry.area
                            best_gc = gc_poly.sort_values("__area", ascending=False).head(1).drop(columns="__area").to_crs(epsg=4326)
                            self._save_boundary_cache(best_gc)
                            return best_gc
                except Exception as e:
                    print(f"   ⚠️ leisure=golf_course lookup failed: {e}")

                # Use coarse boundary if no explicit polygon found
                self._save_boundary_cache(boundary[["geometry"]])
                return boundary[["geometry"]]

            except Exception as e:
                print(f"   ⚠️ OSMnx name search failed for '{variant}': {e}")

        return None

    def _download_golf_features_within_boundary(self, boundary_gdf: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
        try:
            poly = union_all(list(boundary_gdf.geometry))
            print("📥 Fetching golf features inside course boundary via OSMnx…")
            g = self._ox_features_from_polygon(poly, {"golf": True})
            if g is None or g.empty:
                print("⚠️ No golf features returned from OSMnx within boundary")
                return gpd.GeoDataFrame(geometry=[], crs="EPSG:4326")

            # Normalize columns
            for col in ["golf", "leisure", "name", "ref", "par", "handicap"]:
                if col not in g.columns:
                    g[col] = None

            g = g.to_crs(epsg=4326)
            keep = ["geometry", "golf", "leisure", "name", "ref", "par", "handicap"]
            for c in keep:
                if c not in g.columns:
                    g[c] = None
            out = g[keep].copy()
            print(f"✅ Found {len(out)} golf features within boundary")
            return out
        except Exception as e:
            print(f"❌ Error fetching golf features within boundary: {e}")
            return gpd.GeoDataFrame(geometry=[], crs="EPSG:4326")

    def download_golf_courses(self) -> Optional[gpd.GeoDataFrame]:
        """
        Prefer name→boundary (OSMnx) + features-in-area.
        Fallback to bbox-based Overpass if name lookup fails or OSMnx unavailable.
        """
        print("Downloading golf course data… (name→area preferred)")

        # 1) Try cached boundary
        boundary = self._load_boundary_cache()

        # 2) If no cache, try to find by name (OSMnx)
        if boundary is None and OSMNX_AVAILABLE:
            boundary = self._find_course_boundary_by_name()

        # 3) If we have a boundary, fetch all golf=* features within it
        if boundary is not None and not boundary.empty and OSMNX_AVAILABLE:
            features = self._download_golf_features_within_boundary(boundary)
            if features is not None and not features.empty:
                return features
            else:
                print("⚠️ Boundary found but no features; will try bbox fallback.")

        # 4) Fallback: your current Overpass bbox query
        print("🔄 Falling back to bbox-based Overpass query")
...
out skel qt;"""
    ...
    def _osm_to_geodataframe(self, osm_data: Dict[str, Any]) -> gpd.GeoDataFrame:
        ...
        FIXED: Now properly handles multipolygon relations for fairways.
        ...
        # Third pass: process ways and relations
        for element in osm_data['elements']:
            if element['type'] == 'way' and element['id'] in ways:
                way_data = ways[element['id']]
                ...
                if len(coords) >= 2:
                    ...
                    area_golf_types = ['fairway', 'green', 'tee', 'bunker', 'water_hazard',
                                     'lateral_water_hazard', 'rough', 'driving_range']
                    ...
                    # Special handling for golf=hole
                ...
                # Handle multipolygon relations (for fairways with holes for greens)
                tags = element.get('tags', {})
                ...
                if tags.get('type') == 'multipolygon':
                    ...
                    for member in members:
                        if member['type'] == 'way' and member['ref'] in ways:
                            way_coords = ways[member['ref']]['coords']
                            if member['role'] == 'outer':
                                outer_coords = way_coords
                            elif member['role'] == 'inner':
                                inner_coords_list.append(way_coords)

                    # Create polygon with holes if we have valid coordinates
                        ...
                        try:
                            ...
                            for inner in inner_coords_list:
                                if len(inner) >= 3:
                                    ...
                                    closed_inner.append(inner)

                            # Create polygon (with holes if present)
                            if closed_inner:
                                geometry = Polygon(outer_coords, closed_inner)
                            else:
                            ...
                            print(f"Failed to create polygon from relation {element['id']}: {e}")
    ...
    def get_elevation_from_dem(self, grid_resolution: int = 50) -> Optional[Dict[str, Any]]:
        """
        Extract elevation data from DEM (reproject once to EPSG:4326, then sample).
        FIXED: Corrected coordinate grid creation to match flipped elevation data.
        """
        if not RASTERIO_AVAILABLE or not os.path.exists(self.dem_file):
            print("❌ DEM not available for sampling")
            return None
        ...
        try:
            with rasterio.open(self.dem_file) as dem:
                ...
                # Destination grid in EPSG:4326 (matches your bbox + plotting CRS)
                west, south, east, north = self.bbox
                dst_crs = rasterio.crs.CRS.from_epsg(4326)
                dst_height = dst_width = int(grid_resolution)
                ...
                print("   Reprojecting DEM to EPSG:4326 for sampling…")
                reproject(
            ...
            # IMPORTANT: flip so row 0 corresponds to 'north' (for proper matplotlib plotting)
            elevation_grid = np.flipud(dst)

            # FIXED: Build lon/lat grids that match the flipped elevation data
            # After flipping, the first row is north, last row is south
            lons = np.linspace(west, east, dst_width)
            lats = np.linspace(north, south, dst_height)  # FIXED: reversed to north->south after flip
            lon_grid, lat_grid = np.meshgrid(lons, lats)

            # Stats (ignore NaNs)
            valid = elevation_grid[np.isfinite(elevation_grid)]
            if valid.size == 0:
                print("❌ No valid elevation values after reprojection")
                return None
            ...
            elevation_stats = {
                ...
                "elevation_range": float(np.nanmax(valid) - np.nanmin(valid)),
            }
            ...
            print(f"   Mean: {elevation_stats['mean_elevation']:.1f}m")
            print(f"   Range: {elevation_stats['elevation_range']:.1f}m")
            print(f"   Valid points: {valid.size}/{dst_height*dst_width} ({100*valid.size/(dst_height*dst_width):.1f}%)")

            # FIXED: Create elevation points with correct coordinates
            elevation_points = []
            ...
            return {
                "lon_grid": lon_grid,
                "lat_grid": lat_grid,
                "elevation_grid": elevation_grid,  # float32, EPSG:4326 aligned
            ...
            print(f"❌ Error reading/reprojecting DEM: {e}")
            return None
    ...
    def create_elevation_overlay(self, elevation_data: dict, gdf: gpd.GeoDataFrame,
        ...
        FIXED: Simplified to match first script's clean approach.
            ...
            if not gdf.empty:
                ...
                for golf_type, style in color_map.items():
                    ...
                    if not features_subset.empty:
                        if golf_type == 'hole':
                            # Render holes as lines (they usually are LineStrings from OSM)
    ...
    def create_individual_hole_elevation_maps(self, elevation_data: dict, gdf: gpd.GeoDataFrame, hole_profiles: dict) -> bool:
        ...
        FIXED: Simplified to use hole geometry directly from OSM data.
        ...
        try:
            ...
            for idx, hole in holes.iterrows():
                ...
                try:
                    ...
                    # Highlight this specific hole - use the geometry as-is from OSM
                    if hole.geometry.geom_type == 'LineString':
                        # Most holes are already LineStrings from OSM
    ...
    def _get_hole_centerline(self, gdf: gpd.GeoDataFrame, hole_row: pd.Series) -> LineString:
        """
        Return a reasonable centerline for a hole:
          1) use hole_row.geometry if it is a LineString and long enough
          2) else connect nearest tee -> green centroids within local bbox
          3) else fall back to fairway centerline proxy
        """
        geom = hole_row.geometry
        try:
            # 1) Trust a good linestring if it looks like a hole-length feature
            if geom.geom_type == "LineString" and geom.length > 1e-5:
                return geom

            # Local search window
            minx, miny, maxx, maxy = geom.bounds if geom.is_valid else gdf.total_bounds
            pad = 0.001
            bbox = (minx - pad, miny - pad, maxx + pad, maxy + pad)
            local = gdf.cx[bbox[0]:bbox[2], bbox[1]:bbox[3]]

            # 2) Tee → Green
            tees  = local[local["golf"] == "tee"].copy()
            greens = local[local["golf"] == "green"].copy()
            if not tees.empty and not greens.empty:
                tees["cent"]   = tees.geometry.centroid
                greens["cent"] = greens.geometry.centroid
                # pick closest tee/green to the hole geometry centroid
                hc = geom.centroid if geom.is_valid else local.unary_union.centroid
                tees["d"]   = tees["cent"].distance(hc)
                greens["d"] = greens["cent"].distance(hc)
                t_pt = tees.sort_values("d").iloc[0]["cent"]
                g_pt = greens.sort_values("d").iloc[0]["cent"]
                return LineString([t_pt, g_pt])

            # 3) Fairway proxy: longest fairway polygon in the local window
            fair = local[local["golf"] == "fairway"]
            fair = fair[fair.geometry.type.isin(["Polygon", "MultiPolygon"])]
            if not fair.empty:
                fair["_a"] = fair.to_crs(3857).geometry.area
                F = fair.sort_values("_a", ascending=False).iloc[0].geometry
                F = F if F.geom_type == "Polygon" else list(F.geoms)[0]
                bx = F.bounds
                # draw a simple axis across the longest dimension
                if (bx[2]-bx[0]) >= (bx[3]-bx[1]):
                    start, end = (bx[0], F.centroid.y), (bx[2], F.centroid.y)
                else:
                    start, end = (F.centroid.x, bx[1]), (F.centroid.x, bx[3])
        ...
        # Last-ditch fallback
        return geom if geom.geom_type == "LineString" else LineString(geom.envelope.exterior.coords[:2])
    ...
    def run(self) -> bool:
        ...
        # REMOVED: elevation_data = self.process_elevation_data(grid_resolution=240)  # This line was duplicated!
        try:
            elevation_data = self.process_elevation_data()  # Use default grid_resolution=50
            if elevation_data:
